...
class iCaRLmodel:
    ...
    def __init__(self, numclass, feature_extractor, batch_size, task_size, memory_size, epochs, learning_rate, dataset_dir):
        ...
        self.criterion_smoothl1 = nn.SmoothL1Loss(reduction='sum')

        self.dataset_dir = dataset_dir
        self.losses_cls = AverageMeter()
        self.losses_count = AverageMeter()
        self.losses_count_mask = AverageMeter()
        self.losses_branch_distill = AverageMeter()
        self.losses_distill = AverageMeter()
        self.losses_distill_no = AverageMeter()
    ...
    def _get_train_and_val_dataloader(self):
        ...
        self.train_dataset = glob.glob(  # 这里是用于aftertrain使用
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            self.losses_cls.reset()
            self.losses_count.reset()
            self.losses_count_mask.reset()
            self.losses_branch_distill.reset()
            self.losses_distill.reset()
            self.losses_distill_no.reset()
            ...
            print('Loss_cls {loss_cls.avg:.4f}  '
                  'Loss_count {loss_count.avg:.4f}  '
                  'Loss_count_mask {loss_count_mask.avg:.4f}  \n'
                  'Loss_branch_distill {loss_branch_distill.avg:.4f}  '
                  'Loss_distill {loss_distill.avg:.4f}  (old_num:{loss_distill.count})(no_dis:{loss_distill_no.count})'
            .format(loss_cls=self.losses_cls,
                    loss_count=self.losses_count,
                    loss_count_mask=self.losses_count_mask,
                    loss_branch_distill=self.losses_branch_distill,
                    loss_distill=self.losses_distill,
                    loss_distill_no=self.losses_distill_no))
            print('*begin val*')
    ...
    def _compute_loss(self, inputs, st_sizes, points, targets, label):
        self.model.train()
        loss_out, loss_out_mask, loss_branch_distill = 0, 0, 0
        ...
        output, cls, _, output_mask = self.model(inputs, 1)
        label_1 = label
        ...
        for index in range(cls.shape[0]):
            ...
            output_mask_this = output_mask[index:index + 1, channel_num:channel_num + 1, :, :]

            loss_out += self.criterion_bay(prob_list_this, targets_this, output_this)
            loss_out_mask += self.criterion_bay(prob_list_this, targets_this, output_mask_this)
            loss_branch_distill += self.criterion_mse(output_this, output_mask_this.detach())

        loss_branch_distill = loss_branch_distill * 0.1

        self.losses_cls.update(cls_loss.item(), self.batchsize)
        self.losses_count_mask.update(loss_out_mask.item(), self.batchsize)
        self.losses_branch_distill.update(loss_branch_distill.item(), self.batchsize)
        if self.increntmal_phase == 0:

            self.losses_count.update(loss_out.item(), self.batchsize)
            return loss_out + cls_loss + loss_out_mask + loss_branch_distill
        else:
            ...
            for index in range(cls.shape[0]):
                if label_1[index] <= self.increntmal_phase:
                    num_old = num_old + 1

                    channel_num = label_1[index]

                    output_pre = output[index:index + 1, channel_num:channel_num + 1, :, :]
                    output_distill_gt = output[index:index + 1, channel_num:channel_num + 1, :, :]


                    den_distill_gt = output_distill_gt.detach()
                    if torch.sum(den_distill_gt) < 0.1:
                        distill_gt = den_distill_gt
                        num_no_dis += 1
                    else:
                        distill_gt = (den_distill_gt / torch.sum(den_distill_gt)) * len(points[index])
                    loss_distill += 1 - self.criterion_ssim(output_pre, distill_gt)


            loss_distill = loss_distill * alpha
            self.losses_count.update(loss_out.item(), self.batchsize)
            if num_old != 0:
                self.losses_distill.update(loss_distill.item(), num_old)
            if num_no_dis != 0:
                self.losses_distill_no.update(0, num_no_dis)
            return loss_out + loss_distill + cls_loss + loss_out_mask + loss_branch_distill
    ...
    def afterTrain(self):
        ...
        if self.increntmal_phase > 0:
            ...
            self.test_model.output_layer_mask = nn.Conv2d(128, out_channels=self.numclass + 1, kernel_size=1)
    ...
    def _construct_exemplar_set_herding(self, images, m):
        ...
        if self.increntmal_phase == 0:
            ...
            img_other = []
            for index in range(len(images)):
                data = images[index].split('images' + os.sep)[1]  # 【代码修改】原'images/'
                if data.startswith('class1'):
                    img_people.append(images[index])
                else:
                    img_other.append(images[index])
    ...
    def compute_class_mean(self, images):
        ...
        with torch.no_grad():
            ...
            _, _, feature, _ = self.model.feature_extractor(x)
        ...
        for index in range(1, len(img)):
            ...
            with torch.no_grad():
                ...
                _, _, feature_this, _ = self.model.feature_extractor(img[index])
