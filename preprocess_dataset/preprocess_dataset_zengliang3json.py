...
def parse_args():
    ...
    parser.add_argument('--origin-dir', default=r'',
...
if __name__ == '__main__':
    ...
    for phase in ['val_data']:
        ...
        json_list = [os.path.join(args.origin_dir_mat, os.path.basename(jpg_path).replace('.jpg', '.json')) for jpg_path in jpg_list]
        for json_path in json_list:
            ...
            point_list = []
            for p4 in content:
                point = [round((p4[0] + p4[2]) / 2), round((p4[1] + p4[3]) / 2)]
                point_list.append(point)

            points = np.array(point_list).astype(np.float32)
            if phase == 'train_data':
                dis = find_dis(points)
                points = np.concatenate((points, dis), axis=1)

            mat_save_path = os.path.join(sub_save_dir, name)

            gd_save_path = mat_save_path.replace('json', 'npy')
